...
void on_mouse(int event, int x, int y, int flags, void* coordinates) {
  ...
  if(event == EVENT_LBUTTONDOWN){
	...
	rectangle(imageAux, area, Scalar(255, 0, 255), 1, 8, 0);
...
for(;;){
		...
		//If the user loads an empty image, the program ends
		if (image.empty()){
			 std::cerr << "No se ha podido abrir el fichero" << std::endl;
			 return 0;
		}

		//Variables where we storage the points of the mouse
		int coordinates[4];
		int pointA[2];
		int pointB[2];
	  ...
	  namedWindow("image");
		...
		//We wait until the user press ESC
		cv::waitKey(0) & 0xff;
		...
		while (flag == 1)
		{
			if (flag_selected)
			{
				...
				Mat blackWhite;
				image.copyTo(blackWhite); //We copy the user's image
				...
				for (int i=0; i<blackWhite.rows; i++){
					for(int j=0; j<blackWhite.cols; j++){
						//Now we DON NOT want to color in the rectangle's area
						//so if we are not in the rectangle, we color the image with black and white
							if (not (i>pointA[1] and i < pointB[1] and j > pointA[0] and j < pointB[0])) {

								//Type of data of OpenCV that store a pixel of our images
								//We take the pixel of i,j and we change the color.
