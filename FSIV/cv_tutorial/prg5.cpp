...
void on_mouse_testing( int event, int x, int y, int flags, void* param )
{
if (event==cv::EVENT_RBUTTONDOWN) 
{
  ...
  if( xleft<0){
    xleft=0;//check not out of image
  }

  int yleft=y-3;
  
  if( yleft<0){
    yleft=0;//check not out of image
  }
  
  int xright=x+3;
  
  if( xright> image.cols){
    xright=image.cols-1;//check not out of image
  }

  int yright=y+3;
 
  if( yright> image.rows){
    yright=image.rows-1;//check not out of image
  }
...
cv::setMouseCallback( "image", on_mouse_testing, 0 );
//shows image
cv::imshow("image",image);
char c=0;
