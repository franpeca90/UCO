...
void metodoSegundo(){
    ...
    recursivaSegundo(&serie, 0, serie.numeroPuntosSerieTemporal() - 1,eMax);
...
void recursivaSegundo(SerieTemporal *serie, int puntoIzquierda, int puntoDerecha, double eMax){
    ...
    if (error > eMax){
        ...
        posicion = (puntoIzquierda+puntoDerecha)/2;
        serie->dominantePunto(posicion, true); // Primer punto
        // Aplicamos recursividad
        // Por izquierda
        recursivaSegundo(serie, puntoIzquierda, posicion, eMax);
        // Por derecha
        recursivaSegundo(serie, posicion, puntoDerecha, eMax);
    }
