...
void lp::placeStmt::evaluate(){
	/* Se usa la macro que hay en el macro.hpp */
	PLACE( (int) this->izq->evaluateNumber() , (int) this->drch->evaluateNumber() );
}

///////////////////////////////////////////////////////////////////////////////////////////////
///////////////////////////////////////////////////////////////////////////////////////////////

void lp::entireDivisonOperatorNode::print(){
	this->_left->print();
	this->_right->print();
}

float lp::entireDivisonOperatorNode::evaluate(){
	float resultado = 0.0;

	if(this->getType() == NUMBER){
		//Comprobamos que es distinto de cero en base a comparar la diferencia con una cota
		if( std::abs(this->_right->evaluateNumber()) > ERROR_BOUND ){
			resultado = (this->_left->evaluateNumber())/(this->_right->evaluateNumber());
		}
	} else {
		warning("Error de ejecución: La expresión indicada no es numérica", "Divison");
	}
	return resultado;
}
